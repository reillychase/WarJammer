...
class attack(QThread):
    ...
    def pvpgn_login(self):
        ...
        if self.end_flag == 0:
            ...
            print 'attack.pvpgn_login()\n'
            self.connection_status = 0
            self.emit(SIGNAL('catch_status_msg_2(QString, QString)'), '', 'light gray')
            self.emit(SIGNAL('catch_status_msg(QString, QString)'), 'Not Connected', 'light gray')
            return
